...
import '../viewobject/medicine.dart';
...
mixin Utils {
  ...
  static bool isLoginUserEmpty(MasterValueHolder? valueHolder) {
    return valueHolder == null || valueHolder.loginUserId == '';
  }
  ...
  static bool isLogined(MasterValueHolder? valueHolder) {
    if (valueHolder == null || valueHolder.loginUserId == '') {
      return false;
    } else {
      return true;
    }
  ...
  static bool isFavouritedProduct(Medicine data, List<Medicine> dataList) {
    for (Medicine pro in dataList) {
      if (pro.id == data.id) {
        return true;
      }
    }
